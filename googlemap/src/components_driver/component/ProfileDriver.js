...
export default function ProfileDriver(props){
    const {citizenId , Fname ,Lname , birthDate , phone ,plate ,winNo,winName ,profilepicture,passwd ,handleForChangeProfile} = props;
        ...
        if(!!NewPasswdRef.current.value && OldPasswdRef.current.value === passwd){
            if(!!phoneRef.current.value && phoneRef.current.value.length !==10){
                ...
                axios.post(Url.LinkToBackend+"backend/api/editProfile",{
                    ...
                    if(!!phoneRef.current.value){
                        handleForChangeProfile(phoneRef.current.value, NewPasswdRef.current.value)
                        setShowPhone(phoneRef.current.value)
                    }
                    else{
                        handleForChangeProfile(phone, NewPasswdRef.current.value)
                    }
            ...
            if(!!phoneRef.current.value && phoneRef.current.value.length !==10){
                ...
                axios.post(Url.LinkToBackend+"backend/api/editProfile",{
                    ...
                    NotificationManager.success("เปลี่ยนแปลงข้อมูลเสร็จสิ้น",'Alert',1000);
                    console.log(res.data)
                    handleForChangeProfile(phoneRef.current.value, OldPasswdRef.current.value)
                    setShowPhone(phoneRef.current.value)
                    phoneRef.current.value=null;
                    OldPasswdRef.current.value=null;
